<template>
  <section class="w-full">
    <div class="flex flex-col px-0 md:px-12">
      <h2 class="text-lg font-light text-gray-900 tracking-wide text-center mb-8">Trending Now</h2>
      <div
...
const { animes, fetchPopularThisSeason } = useAnimeList();
const emit = defineEmits<{ loaded: [] }>();

onMounted(async () => {
  await fetchPopularThisSeason(1, 6);
  emit("loaded");
});
</script>
